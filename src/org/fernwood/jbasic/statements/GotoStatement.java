...
class GotoStatement extends Statement {
	...
	public Status compile(final Tokenizer tokens) {
		...
		/*
		 * See if it is a line number destination.  If so we must remember
		 * where the token position of the line number is, to support the
		 * RENUMBER command.  Then generate the _GOTO with an integer argument
		 * which the optimizer will later convert to a specific _BR
		 */
		...
		/*
		 * Finally the simplest case; a string label.  Generate a _JMP
		 * to a label, which the optimizer will later convert to a _BR.
		 */
