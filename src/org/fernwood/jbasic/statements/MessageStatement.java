	...
	public Status compile(final Tokenizer tokens) {
		...
		if( tokens.assumeNextToken("USING")) {
			...
			if (!tokens.isIdentifier())
				return status = new Status(Status.INVNAME, code);
			...
			if (tokens.assumeNextToken("(")) {
				...
				if( !tokens.isIdentifier())
					return new Status(Status.EXPLANG);
				if(!tokens.assumeNextToken(")"))
					return new Status(Status.EXPLANG);
			} else
				language = System.getProperty("user.language").toUpperCase();
		...
		if( code == null ) {
			...
			byteCode.add(ByteCode._DEFMSG, 0, code);
