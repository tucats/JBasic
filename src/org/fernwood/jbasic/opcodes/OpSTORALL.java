 ...
 * 
 */
public class OpSTORALL extends AbstractOpcode {
	...
	public void execute(final InstructionContext env) throws JBasicException {
		...
		if (value3.isType(Value.ARRAY)) {
			int count = value2.getInteger();
			for( int ix = 1; ix <= count; ix++ )
				value3.setElement(value1.copy(), ix);
			while( count < value3.size()) {
				value3.removeArrayElement(value3.size());
			}
		} else {
			value3.setElement(value1.copy(), value2.getString().toUpperCase());
		}
		return;
	}
