...
/**
 * BINARY(length) Format<p>
 * 
 * This implements the BINARY input format, which reads a textual
 * representation of a binary integer and converts it to a
 ...
 *
 */
class BinaryFormat extends Informat {
	...
	Value run( InputProcessor input, Command cmd ) {
		...
		while( count < len ) {
			int charpos = InputProcessor.findCharacter(ch, "01");
			if( charpos > 0 ) {
				v = v * 2 + ( charpos-1);
				ch = input.nextChar();
				count++;
