 ...
 *
 * Created on Apr 21, 2011 by cole
 *
 */
...
public class MockSQLStatement {
	...
	private Status prepareDrop(Tokenizer tokens) {
		...
		if( tokens.assumeNextToken("CATALOG")) {
			if( !tokens.testNextToken(Tokenizer.IDENTIFIER))
				return new Status(Status.SQL, new Status(Status.INVCATALOG));
		...
		if( !tokens.assumeNextToken("TABLE"))
			return new Status(Status.SQL, new Status(Status.SQLWORD, "TABLE"));

		if( !tokens.testNextToken(Tokenizer.IDENTIFIER))
			return new Status(Status.SQL, new Status(Status.SQLTABLE));

		String tableName = tokens.nextToken();

		/*
		 * Let's see if it's a catalog reference, in which case we are
		 * going to delete a member from a record rather than delete a
		 * variable.
		 */
		...
		/*
		 * Generate code to test if the item exists and is a TABLE data type.
		 */
	...
	private void generateTypeCheck(String tableName) {
		...
		/*
		 * Generate instruction to test top of stack and signal
		 * an error if the value doesn't match the given type.
		 */
	...
	private Status prepareSelect(Tokenizer tokens) {
		...
		/*
		 * Step one, parse the variable list.  This is either a
		 * list of one or more variable names separated by commas,
		 * or an asterisk indicating that all variables are to be
		 * selected.
		 */

		boolean fCount = false;
		boolean fDistinct = false;
		if( tokens.assumeNextToken("COUNT"))
			fCount = true;
		if(fCount && !tokens.assumeNextSpecial("("))
			return new Status(Status.SQL, new Status(Status.PAREN));

		if( tokens.assumeNextToken("DISTINCT"))
			fDistinct = true;
		...
		/*
		 * Step two, parse the FROM clause which names the source
		 * table.
		 */

		if( !tokens.assumeNextToken("FROM"))
			return new Status(Status.SQL, new Status(Status.SQLWORD, "FROM"));

		/*
		 * Time to start generating code.  If no bytecode stream exists yet, 
		 * go ahead and make one.  It will already exist when this SELECT is
		 * a subordinate clause to another statement like INSERT.
		 */
		...
		if( tableName.equals("SELECT")) {
			...
			if( source.isSingleton && source.isVariable)
				tableName = source.variableName;
			else
				tableName = null;

			/*
			 * Add the code that loads the table we will be selecting from.
			 */
			if( tableName != null ) {
				generateTypeCheck(tableName);
			}
			...
			/*
			 * IS there a WHERE clause for the item (Possibly the right side of a JOIN)
			 */
			...
			if( tokens.assumeNextToken("WHERE")) {
				...
				if( sts.failed())
					return new Status(Status.SQL, sts);
			...
			if( tokens.assumeNextToken("JOIN")) {
				...
				if( tokens.getSpelling().equals("SELECT")) {
					...
					fSelectAll = tempSelectAll;
					fJoin = tempJoin;
					if( fSubSelect && tokens.assumeNextToken(")"))
						fSubSelect = false;
				}
				else {
				...
				//String joinTable = tokens.nextToken();
				//generatedCode.add(ByteCode._LOADREF, joinTable);
				if( joinTable != null )
					generateTypeCheck(joinTable);

				/*
				 * Is there a local WHERE for the right side of the JOIN?
				 */
				...
				if( !tokens.assumeNextToken("ON"))
					return new Status(Status.SQL, new Status(Status.SQLWORD, "ON"));
		...
		/*
		 * Step three, is there a WHERE clause that is global? This cannot happen if
		 * we already have a join in progress.
		 */
		...
		/*
		 * Step four, is there an ORDER BY clause?
		 */
		...
		if( tokens.peek(0).equals("ORDER") && tokens.peek(1).equals("BY")) {
			...
			/* 
			 * The sorts have to be done in the reverse order specified
			 * so we have to build a list of the sorts.
			 */
		...
		/*
		 * Validate the variable list, or fill it in if it was a SELECT * operation.  It 
		 * could also have been a COUNT() operation in which case we really don't care
		 * about column selection at all and just want the row count for the result set.
		 */
		...
		if( fCount && !fDistinct) {
			...
			if( fSelectAll ) {
				if( fDistinct)
					generatedCode.add(ByteCode._CALLF, 1, "DISTINCT");
				if( fCount )
					generatedCode.add(ByteCode._LENGTH);
			}
			else {
				for( int ix = 0; ix < variableList.size(); ix++ ) 
					generatedCode.add(ByteCode._STRING, namePart(variableList.get(ix)));
	...
	private Status prepareInsert( Tokenizer tokens) {
		...
		if(!tokens.assumeNextToken("INTO"))
			return new Status(Status.SQL, new Status(Status.SQLWORD, "INTO"));
		...
		//LValue newTable = new LValue(session, fOptimized);
		//newTable.compileLValue(generatedCode, tokens);
		//newTable.compileLoad();
		...
		/*
		 * The INSERT statement allows a SELECT statement here or a VALUES clause.
		 * Right now we only support VALUES.
		 */
		...
		if( tokens.assumeNextToken("SELECT")) {
			...
			//where.add(ByteCode._STOR, targetName);
			statementType = STMT_INSERT_SELECT;
			return new Status();
		}
		else
			if( tokens.assumeNextToken("VALUES")) {
				if( !tokens.assumeNextSpecial("("))
					return new Status(Status.SQL, new Status(Status.PAREN));
				...
				while(true) {
					...
					if( tokens.assumeNextSpecial(")"))
						break;
					if(!tokens.assumeNextSpecial(","))
						return new Status(Status.SQL, new Status(Status.COMMA));

				}

				generatedCode.add(ByteCode._ARRAY, vCount);
				generatedCode.add(ByteCode._INSERT);
				//where.add(ByteCode._STOR, tableName);
				...
				if( tokens.assumeNextToken("VALUE")) {
					if( !tokens.assumeNextSpecial("("))
						return new Status(Status.SQL, new Status(Status.PAREN));
					Expression dv = new Expression(this.session);
					Status sts = dv.compile(generatedCode, tokens);
	...
	private Status prepareCreate( Tokenizer tokens) {
		...
		if( tokens.assumeNextToken("CATALOG")) {
			String catalog;
			if( !tokens.testNextToken(Tokenizer.IDENTIFIER))
				return new Status(Status.INVCATALOG, tokens.nextToken());
		...
		/*
		 * Is it a straight-up table create?
		 */
		if( tokens.assumeNextSpecial("(")) {
			...
			while( tokens.testNextToken(Tokenizer.IDENTIFIER)) {
				...
				if( namesInUse.get(varName) != null)
					return new Status(Status.SQL, new Status(Status.SQLDUPFIELD, varName));
					...
					if( type.equals("DOUBLE"))
						...
						if( tokens.assumeNextToken("(")) {
							if( !tokens.testNextToken(Tokenizer.INTEGER))
								return new Status(Status.SQL, new Status(Status.SQLWORD, tokens.nextToken()));
							tokens.nextToken(); /* Skip the size, we don't use it now */
							if( !tokens.testNextToken(")"))
								return new Status(Status.SQL, new Status(Status.PAREN));
						}
						type="@STRING";
					} else 
						return new Status(Status.SQL, new Status(Status.SQLWORD, type));	
				var.append(type);
				count++;
			...
			for( int ix = 0; ix < variableList.size(); ix++)
				generatedCode.add(ByteCode._STRING, variableList.get(variableList.size() - ix - 1));
		...
		/*
		 * Or is it a CREATE TABLE LIKE or CREATE TABLE AS?
		 */
		if( tokens.peek(0).equals("AS"))
			createType = STMT_CREATE_AS;
		else
			if( tokens.peek(0).equals("LIKE"))
				createType = STMT_CREATE_LIKE;

		if( createType == STMT_UNKNOWN )
			return new Status(Status.SQL, new Status(Status.SQLWORD, "LIKE or AS"));
		tokens.nextToken();

		if( !tokens.assumeNextToken("SELECT"))
			return new Status(Status.SQL, new Status(Status.SQLWORD, "SELECT"));
	...
	public Status execute(SymbolTable symbolTable) {
		...
		/*
		 * If the statement type isn't known, it wasn't successfully prepared.
		 */
		if( statementType == STMT_UNKNOWN)
			return new Status(Status.SQL, new Status(Status.SQLPREP));

		/*
		 * If the code can be compiled completely, then let's execute it now.  Examples
		 * would be the SELECT or DROP statement
		 */
		if( generatedCode != null ) {
			...
			if( symbolTable.getBoolean("SYS$SQL_DISASM"))
				generatedCode.disassemble();
	...
	public Value fetch( int maxResultSetSize) {
		...
		if( maxResultSetSize == -1)
			resultSetSize = ( table == null ? 0 : table.size());
		else
	...
	public Value fetch() {
		...
		if( statementType != STMT_SELECT && statementType < STMT_CREATE
				&& statementType != STMT_CREATE_TABLE) 
			return null;
		...
		/*
		 * Empty out the result set table so we're ready to add new
		 * elements to it.
		 */
		resultSet.empty();
